...
begin
  ...
  TSimpleSQL<TUSUARIO>
    .New(nil)
    .Where('CPF_USUARIO = ' + QuotedStr(aCPF) + ' AND SENHA = ' + QuotedStr(UpperCase(CodificarSenha(aSenha))))
    .Select(LSQL);
  FConn.SQL.Add(LSQL);

  if not (FConn.Open.DataSet.IsEmpty) then
    Result:= FConn.DataSet.AsJSONObject;
end;
